...
- **Function-name resolution** (spec v0.11.0) — `visitFunctionCall` resolves ignored check → **user-defined functions** → `FAIL`/`UNWRAP` → `this.functions` (builtins + externals). Script functions shadow everything (this runtime was builtins-first until spec v0.11.0 pinned script-first; fixture 104 guards the order — don't move the user-defined block back down).
