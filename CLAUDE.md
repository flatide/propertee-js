...
- **Static validation pass** (ProperTee issue #9; host API, no language change) — `visitor.validate(tree)` scans the whole tree (dead branches included) and returns `"line L:C: 'X' is not available in this environment"` entries for hidden-keyword constructs / ignored-function calls. Runtime checks unchanged (backstop). Tested by `test/run_validate_test.js`, wired into `run_tests.sh` as a host-API test (not a numbered fixture — the numbered namespace belongs to the shared conformance suite).
